...
class GammaScalpingTask:
    ...
    def initialize(
        ...
        call_price = engine.bs_price(S, np.array([K]), np.array([T]), np.array([sigma]), option_type="call")[0]
        put_price = engine.bs_price(S, np.array([K]), np.array([T]), np.array([sigma]), option_type="put")[0]
        
        call_delta = engine.delta(S, np.array([K]), np.array([T]), np.array([sigma]), option_type="call")[0]
        put_delta = engine.delta(S, np.array([K]), np.array([T]), np.array([sigma]), option_type="put")[0]
        ...
        call_theta = engine.theta(S, np.array([K]), np.array([T]), np.array([sigma]), option_type="call")[0]
        put_theta = engine.theta(S, np.array([K]), np.array([T]), np.array([sigma]), option_type="put")[0]
    ...
    def score(self, episode_history: List[Any], state: VSRState) -> float:
        ...
        if len(spot_moves) > 2 and len(hedge_quantities) > 2:
            ...
            if np.std(spot_moves) > 1e-6 and np.std(hedge_quantities) > 1e-6:
                ...
                timing_score = max(0.0, (correlation + 1.0) / 2.0)  # Normalize to [0, 1]
            else:
                timing_score = 0.5
        else:
            timing_score = 0.5
